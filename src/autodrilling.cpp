...
bool AutoDrillingStateMachine::beforeStateChange(const QString& oldState, const QString& newState) {
    logInfo(QString("准备切换状态: %1 -> %2").arg(oldState).arg(newState));
...
void AutoDrillingStateMachine::afterStateChange(const QString& oldState, const QString& newState) {
    logInfo(QString("状态已切换: %1 -> %2").arg(oldState).arg(newState));
    
    // 发送信号以更新GUI
    emit currentStepChanged(newState);
    
    // 特殊状态转换处理
    if (newState == STATE_PIPE_INSTALLATION_LOOP && oldState != STATE_PIPE_INSTALLATION_LOOP) {
        // 进入钻管安装循环，增加钻管计数
        incrementPipeCount();
    } else if (newState == STATE_PIPE_REMOVAL_LOOP && oldState != STATE_PIPE_REMOVAL_LOOP) {
        // 进入钻管拆卸循环，减少钻管计数
        decrementPipeCount();
    }
}
...
bool RobotArmStateMachine::setRotationPosition(int position) {
    ...
    // 计算旋转角度：0度表示对准钻进机构，180度表示对准存储单元
    float angle = position * 180.0f;
